...
this.ctx.fillRect(0, 0, this.config.canvasWidth, this.config.canvasHeight);
  ...
  drawTree: function(plant) {
    ...
    // Branches
    const branchCount = 3 + Math.floor(plant.age);
    for (let i = 0; i < branchCount; i++) {
      const branchHeight = plant.y - size * (0.3 + (i * 0.15));
      const branchLength = size * 0.3 * (1 - (i * 0.1));
      const angle = (i % 2 === 0) ? Math.PI / 4 : -Math.PI / 4;
      
      ctx.beginPath();
      ctx.moveTo(plant.x, branchHeight);
      ctx.lineTo(plant.x + Math.cos(angle) * branchLength, branchHeight + Math.sin(angle) * branchLength);
      ctx.lineWidth = size * 0.05;
      ctx.strokeStyle = '#5D4037';
      ctx.stroke();
    }
