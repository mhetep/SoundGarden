...
const AudioGarden = {
    ...
    gltfLoader: null, // Three.js GLTF loader
    ...
    setupCanvas: function() {
      ...
      window.addEventListener('resize', () => {
        ...
        this.canvas.height = this.config.canvasHeight;
        this.redrawGarden();
        this.renderer.setSize(this.config.canvasWidth, this.config.canvasHeight); // Adjust 3D renderer
        this.camera.aspect = this.config.canvasWidth / this.config.canvasHeight;
    ...
    addPlant: function(x, y, size, frequency) {
      ...
      if (plantType.name === 'flower') {
        if (Math.random() < 0.5) {
          plant.modelName = null; // Existing flower drawing
        } else {
          plant.modelName = 'anemone';
        }
      } else if (plantType.name === 'cactus') {
        if (Math.random() < 0.5){
          plant.modelName = null;
        } else {
          plant.modelName = 'cactus_3d';
        }
      }
    ...
    varyColor: function(baseColor) {
      ...
      // Convert back to hex
      return `#<span class="math-inline">\{<2\>Math\.floor\(newR\)\.toString\(16\)\.padStart\(2, '0'\)\}</span>{Math.floor(newG).toString(16).padStart(2, '0')}${Math.floor(newB).toString(16).padStart(2, '0')}`;
    },
    ...
    // Setup 3D Scene
    setup3DScene: function() {
      this.scene = new THREE.Scene();
      this.camera = new THREE.PerspectiveCamera(75, this.config.canvasWidth / this.config.canvasHeight, 0.1, 1000);
      this.renderer = new THREE.WebGLRenderer({ canvas: this.canvas, alpha: true }); // Use existing canvas, enable alpha
      this.renderer.setSize(this.config.canvasWidth, this.config.canvasHeight);
      this.camera.position.set(0, -10, 10); // Adjust camera position
    ...
    loadModels: function() {
      this.models = {};
      this.gltfLoader.load('anemone_flower_low_poly.glb', (gltf) => {
        this.models['anemone'] = gltf.scene;
      });
      this.gltfLoader.load('cactus_reality_scan.glb', (gltf) => {
        this.models['cactus_3d'] = gltf.scene;
      });
    },

    // Render 3D Model
    draw3DModel: function(plant) {
      if (!this.models[plant.modelName]) return;

      const model = this.models[plant.modelName].clone(); // Clone to avoid modifying the original

      // Calculate 3D position (approximate)
      const worldY = this.config.canvasHeight - plant.y; // Invert Y for Three.js
      model.position.set(plant.x - this.config.canvasWidth / 2, -worldY + this.config.canvasHeight / 2, 0); // Center
      model.scale.set(plant.currentSize * 0.1, plant.currentSize * 0.1, plant.currentSize * 0.1); // Adjust scale

      model.rotation.y = plant.rotation;
